...
w <- (alldata) * invCov %*% (c1mean - c0mean)

#calculate the threshold
w0 <- (alldata * t(c1mean) %*% invCov %*% c1mean)/2 - (alldata * t(c0mean) %*% invCov %*% c0mean)/2 +
  log(data1/alldata) - log(data0/alldata)
...
ggsave(plotGroundTruth, file = "Fisher's Ground Truth.PNG", width = 20, height = 10, units = "cm")
...
plotLDA <- ggplot(dfLDA, aes(X, Y, color=factor(class))) + geom_point(aes(x = X, y = Y),size=2) +
  geom_vline(xintercept = w0, linetype = "dotted", color = "blue", size = 1.5)
ggsave(plotLDA, file = "Fisher's.PNG", width = 20, height = 10, units = "cm")
